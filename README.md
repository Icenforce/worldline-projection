...
The current prototype already covers the deterministic substrate, accountable placement for settlements/lumber camps/mines/roads/forts/ruins/battlefields, the timber-collapse perturbation path, route-cut perturbation coverage, generalized compaction retention checks for the current proof paths, and the initial negative-control comparison against Control C. Remaining contract work is mainly broader gate-audit closure and validation hardening beyond the current proof paths.
